...
//
import SessionMemoryStore from './store/SessionMemoryStore';
import initializeCompanies from '../utils/company';
//
import type { User } from '../types/user';
import type { Company } from '../types/company';

let _userStore: LocalForage | SessionMemoryStore<User>;
let _companyStore: LocalForage | SessionMemoryStore<Company>;
    ...
    async init() {
      if (!_companyStore) {
        _companyStore = isJest()
          ? new SessionMemoryStore<Company>()
          : localForage.createInstance({
              driver: localForage.INDEXEDDB,
              name: 'nu_companys',
            });
