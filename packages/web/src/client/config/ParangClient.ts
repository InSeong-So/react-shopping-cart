...
class ParangClient {
  ...
  public post = <T = unknown>(url: string, data?: any, config?: Options): Promise<Response<T>> => {
    return request<T>(url, config || {}, 'post', data, this.defaults);
  };

  public put = <T = unknown>(url: string, data?: any, config?: Options): Promise<Response<T>> => {
    return request<T>(url, config || {}, 'put', data, this.defaults);
  };

  public patch = <T = unknown>(url: string, data?: any, config?: Options): Promise<Response<T>> => {
    return request<T>(url, config || {}, 'patch', data, this.defaults);
  };
...
export default new ParangClient();
